...
$sql .= "INNER JOIN prstshp_orders po ON po.id_cart = pc.id_cart AND po.payment = 'Referencia' ";
...
$sql .= "WHERE po.id_order NOT IN (SELECT id_order FROM lista_ordenes_referencia)";
...
foreach ($orderInformation as $item) {
    ...
    $sql_insert = "INSERT INTO lista_ordenes_referencia VALUES (null,{$id_order},{$id_cart},'{$carrier}','{$customerID}','{$pay}','{$payment}',{$cotizacion},{$orden_venta},{$pago_aprobado},{$diario},'{$numero_diario}',{$liberacion},{$correo},{$paqueterias},'{$ov}',{$error_ov},'{$cot}','{$sitio}',{$encarrier},{$guia},'{$guiaspaq}')";
