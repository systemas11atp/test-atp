...
$sql          = "SELECT * FROM lista_ordenes_referencia WHERE cotizacion = 1 AND orden_venta = 0 LIMIT 5";
    ...
    foreach ($cots as $cot) {
        ...
        if ($err) {
            ...
            if ($err) {
                ...
                $sql_update = "UPDATE lista_ordenes_referencia SET ov = IF(ov != '',CONCAT(ov,'_','{$orventa}'),'{$orventa}'),  orden_venta = 1 WHERE id_lista_orden_referencia = {$cotizacion['id_lista_orden_referencia']}";
                Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql_update);
                capuraLogs::nuevo_log("traerCotozaciones _referencia sql_update : {$sql_update}");
