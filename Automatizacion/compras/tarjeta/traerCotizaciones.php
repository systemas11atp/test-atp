...
$sql          = "SELECT * FROM lista_ordenes_tarjeta WHERE cotizacion = 1 AND orden_venta = 0 LIMIT 5";
    ...
    foreach ($cots as $cot) {
        ...
        if ($err) {
            ...
            if ($err) {
                ...
                $sql_update = "UPDATE lista_ordenes_tarjeta SET ov = IF(ov != '',CONCAT(ov,'_','{$orventa}'),'{$orventa}'),  orden_venta = 1 WHERE id_lista_orden_tarjeta = {$cotizacion['id_lista_orden_tarjeta']}";
                Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql_update);
                capuraLogs::nuevo_log("traerCotozaciones _tarjeta sql_update : {$sql_update}");
