...
class AddressCore extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'colony' => ['type' => self::TYPE_STRING, 'required' => true, 'size' => 128],
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://prod-111.westus.logic.azure.com:443/workflows/77ff7b83877d4086a36c8301011f7402/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=teQsRuKLXvTzhUXtoYNniB2JE6vvuEisnXnnAzpI3RA",
            ...
            CURLOPT_POSTFIELDS => "{\"n_cid\": \"$this->id\"}",
    ...
    public function update($null_values = false){
        ...
        capuraLogs::nuevo_log("Address update id : {$this->id}");
        capuraLogs::nuevo_log("Address update id_customer : {$this->id_customer}");
        $customerID = Db::getInstance()->getValue('SELECT customerID FROM ' . _DB_PREFIX_ . 'customer WHERE id_customer = '.$this->id_customer);
        $addressID = Db::getInstance()->getValue('SELECT addressID FROM ' . _DB_PREFIX_ . 'address WHERE id_address = '.$this->id);
        $effective = Db::getInstance()->getValue('SELECT effective FROM ' . _DB_PREFIX_ . 'address WHERE id_address = '.$this->id);
        ...
        $POSTFIELDS = "{\"id_address\":\"$this->id\",\"customerID\" : \"$customerID\",\"addressID\" : \"$addressID\",\"effective\" : \"$effective\", \"accion\": \"actualizar\"}";
        capuraLogs::nuevo_log("Address update url : {$url}");
        capuraLogs::nuevo_log("Address update POSTFIELDS : {$POSTFIELDS}");
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url,
            CURLOPT_RETURNTRANSFER => true,
        ...
        $responseP = curl_exec($curl);
        $err = curl_error($curl);
        capuraLogs::nuevo_log("Address update responseP : {$responseP}");
        capuraLogs::nuevo_log("Address update err : {$err}");
        curl_close($curl);
    ...
    public function delete()
    {
        ...
        capuraLogs::nuevo_log("Address delete id : {$this->id}");
        capuraLogs::nuevo_log("Address delete id_customer : {$this->id_customer}");
        $customerID = Db::getInstance()->getValue('SELECT customerID FROM ' . _DB_PREFIX_ . 'customer WHERE id_customer = '.$this->id_customer);
        $addressID = Db::getInstance()->getValue('SELECT addressID FROM ' . _DB_PREFIX_ . 'address WHERE id_address = '.$this->id);
        $effective = Db::getInstance()->getValue('SELECT effective FROM ' . _DB_PREFIX_ . 'address WHERE id_address = '.$this->id);
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://prod-45.westus.logic.azure.com:443/workflows/15d7da7d98bf4b2081211b9fdab1d63c/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=G28Xkh8fQ0GlsGWcnM9WzWNuIsbpFxdyoX3XYLpCXw0",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
            CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
            CURLOPT_CUSTOMREQUEST => "POST",
            CURLOPT_POSTFIELDS => "{\"id_address\" : \"$this->id\",\"customerID\" : \"$customerID\",\"addressID\" : \"$addressID\",\"effective\" : \"$effective\", \"accion\": \"eliminar\"}",
            CURLOPT_HTTPHEADER => array("content-type: application/json"),
        ));
        $responseP = curl_exec($curl);
        $err = curl_error($curl);
