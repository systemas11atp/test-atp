...
abstract class ProductListingFrontControllerCore extends ProductPresentingFrontController
{
    ...
    protected function getProductSearchVariables()
    {
        ...
        if (Tools::getValue('page') != "") {
            $pageIndex = Tools::getValue('page');
            $suma      = ($pageIndex - 1) * 10;
            $itemsTo   = $pageIndex * 10;
        }
        $startIndex = 1 + $suma;

        $itemsFrom = 1 + $suma;

        $key          = "AIzaSyCCKQd4g1GhvGhMtM8ut6ccQrSc7mkOl1M";
        $searchEngine = "6c09f2d7523f06310";
        $curl         = curl_init();

        curl_setopt_array($curl, [
            CURLOPT_URL            => "https://www.googleapis.com/customsearch/v1?key=" . $key . "&cx=" . $searchEngine . "&q=" . $search_string . "&start=" . $startIndex,
            ...
            CURLOPT_POSTFIELDS     => "",
            CURLOPT_COOKIE         => "NID=216%3DNkY_ZSDYyGubnhOVpgiGLb6a_rI6MyGxkePT5N9g5MUrEs9cpjOEdiqBi6XltHpQonApXNec0YOE4tJNsCzrWD59de9BBW7jNNoXgUbhi89iDJsw7LFlX9recIivzYUPaIFBRwmBwYmg6z5ZjgOXrhCo57Tv-l7oc-OrMyEDiSU",
            CURLOPT_HTTPHEADER     => [
        ...
        $arrayApiSearch = [];
        $count          = 0;
        $pageCount      = (int) $response->queries->request[0]->totalResults / 10;
        //print_r(round($test));
        $pagination[total_items]      = $response->queries->request[0]->totalResults;
        $pagination[items_shown_from] = $itemsFrom;
        $pagination[items_shown_to]   = $itemsTo;
        $pagination[pages_count]      = round($pageCount);
        //print_r($pagination);
        foreach ($response->items as $resp) {
            $arrayApiSearch[$count][name]                                     = $resp->title;
            $arrayApiSearch[$count][url]                                      = $resp->link;
            $arrayApiSearch[$count][show_price]                               = 0;
            $arrayApiSearch[$count][default_image][bySize][home_default][url] = $resp->pagemap->cse_image[0]->src;
            $count++;
        }
        //print_r($pagination);
        //default_image.bySize.home_default.url
        //print_r($products[1]->default_image[bySize][home_default]);
        $searchVariables = [
            'result'                  => $result,
            ...
            'js_enabled'              => $this->ajax,
            'products'                => $arrayApiSearch,
            //'arrayApiSearch'          => $arrayApiSearch,
