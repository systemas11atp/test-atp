...
class CarrierCore extends ObjectModel
{
    ...
    public function getDeliveryPriceByWeight($total_weight, $id_zone)
    {
        ...
        if (!isset(self::$price_by_weight[$cache_key])) {
            ...
            if (!isset($result['price'])) {
                ...
                //self::$price_by_weight[$cache_key] = 123456;
            }
            if ($this->id_reference == 3) {
                $context = Context::getContext();
                $cart    = $context->cart;

                $data = '{"cartId":' . $cart->id . ',"addressId":"' . $cart->id_address_delivery . '"}';
                //capuraLogs::nuevo_log("Carrier data ::: {$data}");
                //capuraLogs::nuevo_log("Carrier this ::: ".json_encode($this)."");
                //capuraLogs::nuevo_log("Carrier cart->id ::: ".$cart->id."");
                //capuraLogs::nuevo_log("Carrier cart->id_address_delivery ::: ".$cart->id_address_delivery."");
                $curl = curl_init();
                curl_setopt_array($curl, array(
                    CURLOPT_PORT           => "",
                    CURLOPT_URL            => "https://avanceytec.com.mx/Automatizacion/paqueterias/calculapaqueteria.php",
                ...
                //capuraLogs::nuevo_log("CARRIER response ::: {$response}");
                $response = json_decode($response);
                //capuraLogs::nuevo_log("Carrier response ::: {$response->sumatoriaPrcios}");

                if ($response->sumatoriaPrcios > 0) {
                    self::$price_by_weight[$cache_key] = $response->sumatoriaPrcios;
                } else {
                    self::$price_by_weight[$cache_key] = 0;
                }
                print_r("Hola");
                print_r(self::$price_by_weight[$cache_key]);

                //self::$price_by_weight[$cache_key] = 90000;

            }
            //capuraLogs::nuevo_log("Carrier getDeliveryPriceByWeight :: {$sql}");
        }
        ...
        //capuraLogs::nuevo_log("Carrier 1 price_by_weight :: {$price_by_weight}");
        //capuraLogs::nuevo_log("Carrier 1.2 price_by_weight :: {$price_by_weight[$cache_key]}");
    ...
    public static function checkDeliveryPriceByWeight($id_carrier, $total_weight, $id_zone)
    {
        ...
        //capuraLogs::nuevo_log("Carrier 2 price_by_weight :: {$price_by_weight}");
        //capuraLogs::nuevo_log("Carrier 2.2 price_by_weight :: {$price_by_weight[$cache_key]}");
    ...
    {
        $id_carrier = (int) $this->id;
        //capuraLogs::nuevo_log("Carrier getDeliveryPriceByPrice :: {$id_carrier}");
        ...
        //capuraLogs::nuevo_log("Carrier 3 price_by_price :: {$price_by_price}");
        //capuraLogs::nuevo_log("Carrier 3.2 price_by_price :: {$price_by_price[$cache_key]}");
    ...
    public static function checkDeliveryPriceByPrice($id_carrier, $order_total, $id_zone, $id_currency = null)
    {
        ...
        //capuraLogs::nuevo_log("Carrier 4 price_by_price :: {$price_by_price}");
        //capuraLogs::nuevo_log("Carrier 4.2 price_by_price :: {$price_by_price[$cache_key]}");
    ...
    {
        //capuraLogs::nuevo_log("Carrier function");
    ...
    public static function getCarriersForOrder($id_zone, $groups = null, $cart = null, &$error = [])
    {
        ...
        $shipping_method_implode = json_encode($result);
        $results_array           = [];
        foreach ($result as $k => $row) {
            ...
            $unSitio = false;
            $sql     = "SELECT * FROM prstshp_cart_product WHERE id_cart = {$cart->id} GROUP BY sitio";
            $sitios  = Db::getInstance()->executeS($sql);
            if (count($sitios) == 1) {
                $sql = "SELECT * FROM prstshp_cart_product pcp  ";
                $sql .= "INNER JOIN prstshp_cart pc ON pc.id_cart = pcp.id_cart  ";
                $sql .= "INNER JOIN prstshp_customer pcu ON pcu.id_customer = pc.id_customer AND pcu.sitio = pcp.sitio ";
                $sql .= "WHERE pc.id_cart =  {$cart->id}  GROUP BY pcp.sitio ";
                if (Db::getInstance()->executeS($sql)) {
                    $unSitio = true;
                }
            }
            $row['name'] = ((string) ($row['name']) != '0' ? $row['name'] : Carrier::getCarrierNameFromShopName());
            //capuraLogs::nuevo_log("Carrier name : -{$row['name']}-");
            ...
            //capuraLogs::nuevo_log("--------------------- Carrier price : -{$row['price']}-");
            if (!$unSitio && (int) $row['id_reference'] < 3) {
                $row['price'] = false;
            }
            //capuraLogs::nuevo_log("Carrier price : -{$row['price']}-");
            //capuraLogs::nuevo_log("Carrier id_reference : -{$row['id_reference']}-");
            ...
            //capuraLogs::nuevo_log("Carrier price_tax_exc : -{$row['price_tax_exc']}-");
            $row['img'] = file_exists(_PS_SHIP_IMG_DIR_ . (int) $row['id_carrier'] . '.jpg') ? _THEME_SHIP_DIR_ . (int) $row['id_carrier'] . '.jpg' : '';
            //capuraLogs::nuevo_log("Carrier img : -{$row['img']}-");
            //capuraLogs::nuevo_log("Carrier cart_id : -{$cart->id}-");
            /*
            ...
            if ($row['price'] === false) {
                //capuraLogs::nuevo_log("Carrier false ? id_reference : -{$row['id_reference']}-");
                unset($result[$k]);
