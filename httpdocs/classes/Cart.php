...
FROM (' . $firstUnionSql . ' UNION ' . $secondUnionSql . ') as q';
    ...
    /*Actualizacion sitios carrito*/
    public function actualiza_sitios()
    {
        $cart_id      = (int) $this->id;
        $sitioCliente = "";
        $sql          = "SELECT pc.sitio FROM " . _DB_PREFIX_ . "customer pc INNER JOIN " . _DB_PREFIX_ . "cart pct ON pct.id_customer = pc.id_customer AND  pct.id_cart = {$cart_id}";
        $sitios       = Db::getInstance()->executeS($sql);
        $sitioCliente = $sitios[0]['sitio'];
        //capuraLogs::nuevo_log("1) Cart sql : {$sql}");
        //capuraLogs::nuevo_log("2) Cart sitioCliente : {$sitioCliente}");
        if ($sitioCliente == "") {
            $sitioCliente = "CHIH";
        }
        $sql = "SELECT * FROM " . _DB_PREFIX_ . "cart_product WHERE id_cart = {$cart_id} GROUP BY sitio";
        //capuraLogs::nuevo_log("3) Cart sql : {$sql}");
        $totalSitios = 0;
        $sitios      = Db::getInstance()->executeS($sql);
        if ($sitios) {
            $totalSitios = count($sitios);
        }
        //capuraLogs::nuevo_log("4) Cart totalSitios : {$totalSitios}");
        if ($totalSitios > 1) {
            $sql       = "SELECT * FROM " . _DB_PREFIX_ . "cart_product WHERE id_cart = {$cart_id}";
            $productos = Db::getInstance()->executeS($sql);
            //capuraLogs::nuevo_log("5) Cart sql : {$sql} (".count($productos).")");
            $deOrigen = true;
            foreach ($productos as $producto) {
                $id_product           = $producto['id_product'];
                $id_product_attribute = $producto['id_product_attribute'];
                $quantity             = $producto['quantity'];
                $sql                  = "SELECT * FROM " . _DB_PREFIX_ . "stock_global WHERE id_product = {$id_product} AND id_product_attribute ={$id_product_attribute} AND quantity >= {$quantity} AND sitio = '{$sitioCliente}'";
                //capuraLogs::nuevo_log("6) Cart sql : {$sql}");
                if (!Db::getInstance()->executeS($sql)) {
                    //capuraLogs::nuevo_log("7) Cart NOOOOO!");
                    $deOrigen = false;
                }
            ...
            if (!$deOrigen) {
                ...
                foreach ($sitios as $sitio) {
                    $sitioLocal = $sitio['sitio'];
                    $cuentele   = 0;
                    $sitiosStr  = "";
                    $coma       = "";
                    //capuraLogs::nuevo_log("8) Cart sql : {$sql} (".count($productos).")");
                    foreach ($productos as $producto) {
                        $id_product           = $producto['id_product'];
                        $id_product_attribute = $producto['id_product_attribute'];
                        $quantity             = $producto['quantity'];
                        $sql                  = "SELECT * FROM " . _DB_PREFIX_ . "stock_global WHERE id_product = {$id_product} AND id_product_attribute ={$id_product_attribute} AND quantity >= {$quantity} AND sitio = '{$sitioLocal}'";
                        //capuraLogs::nuevo_log("9) Cart sql : {$sql}");
                        if (Db::getInstance()->executeS($sql)) {
                            $sitioAct = $producto['sitio'];
                            $sitiosStr .= "{$coma}({$id_product},{$id_product_attribute})";
                            $sql = "INSERT INTO prstshp_sitios_productos VALUES({$cart_id},{$id_product},{$id_product_attribute},{$quantity},'{$sitioAct}','{$sitioLocal}','')";
                            Db::getInstance()->execute($sql);
                            //capuraLogs::nuevo_log("10) Cart sql : {$sql}");
                            $coma = ",";
                            $cuentele++;
                        } else {
                            $sql           = "SELECT * FROM " . _DB_PREFIX_ . "stock_global WHERE id_product = {$id_product} AND id_product_attribute ={$id_product_attribute} AND quantity >= {$quantity} ORDER BY quantity DESC LIMIT 1";
                            $nuevos_sitios = Db::getInstance()->executeS($sql);
                            $nuevoSitio    = $nuevos_sitios[0]['sitio'];
                            $sitioAct      = $producto['sitio'];
                            $sitiosStr .= "{$coma}({$id_product},{$id_product_attribute})";
                            $sql = "INSERT INTO prstshp_sitios_productos VALUES({$cart_id},{$id_product},{$id_product_attribute},{$quantity},'{$sitioAct}','{$nuevoSitio}','')";
                            Db::getInstance()->execute($sql);
                            //capuraLogs::nuevo_log("11) Cart sql : {$sql}");
                            $coma = ",";
                        }
                    }
                    if ($cuentele == count($productos)) {
                        //capuraLogs::nuevo_log("12) Cart Todos  salen de : {$sitioLocal}");
                        $unicoSitio = $sitioLocal;
                        break;
                    } else {
                        //capuraLogs::nuevo_log("13) Cart sitioLocal :($cuentele) {$sitioLocal} [{$sitiosStr}]");
                    }
                }
                if ($unicoSitio == "") {
                    $sql    = "SELECT *, COUNT(*) as total FROM prstshp_sitios_productos WHERE id_cart = {$cart_id} GROUP by sitio_existente order by total desc";
                    $unicos = Db::getInstance()->executeS($sql);
                    foreach ($unicos as $unico) {
                        $sitioExistente = $unico['sitio_existente'];
                        $sql            = "SELECT * FROM prstshp_sitios_productos WHERE id_cart = {$cart_id} AND sitio_existente = '{$sitioExistente}'";
                        //capuraLogs::nuevo_log("14) Cart sql : {$sql}");
                        $nproductos = Db::getInstance()->executeS($sql);
                        foreach ($nproductos as $nproducto) {
                            $id_product           = $nproducto['id_product'];
                            $id_product_attribute = $nproducto['id_product_attribute'];
                            $sql                  = "UPDATE prstshp_sitios_productos SET sitio_fijo = '{$sitioExistente}' WHERE id_cart = {$cart_id} AND id_product = {$id_product} AND id_product_attribute = {$id_product_attribute} AND sitio_fijo = ''";
                            Db::getInstance()->execute($sql);
                            //capuraLogs::nuevo_log("15) Cart sql : {$sql}");
                        }
                    }
                    $sql = "SELECT * FROM prstshp_sitios_productos WHERE id_cart = {$cart_id} GROUP BY id_product, id_product_attribute";
                    //capuraLogs::nuevo_log("16) Cart sql : -{$sql}-");
                    //capuraLogs::nuevo_log("17) Cart unicoSitio : -{$unicoSitio}-");
                    $unicos = Db::getInstance()->executeS($sql);
                    foreach ($unicos as $unico) {
                        $id_product           = $unico['id_product'];
                        $id_product_attribute = $unico['id_product_attribute'];
                        $sitio_fijo           = $unico['sitio_fijo'];
                        $sql                  = "UPDATE " . _DB_PREFIX_ . "cart_product SET sitio = '{$sitio_fijo}' WHERE id_cart = {$cart_id} AND id_product = {$id_product} AND id_product_attribute = {$id_product_attribute} ";
                        //capuraLogs::nuevo_log("18) Cart sql : -{$sql}-");
                        Db::getInstance()->execute($sql);
                    }
                }
                //capuraLogs::nuevo_log("19) Cart unicoSitio : -{$unicoSitio}-");
            } else {
                $sql_update = "UPDATE " . _DB_PREFIX_ . "cart_product SET sitio = '{$sitioCliente}' WHERE id_cart = {$cart_id}";
                Db::getInstance()->execute($sql_update);
                //capuraLogs::nuevo_log("20) Cart sql_update : -{$sql_update}-");
            }
            if ($unicoSitio != "") {
                $sql_update = "UPDATE " . _DB_PREFIX_ . "cart_product SET sitio = '{$unicoSitio}' WHERE id_cart = {$cart_id}";
                Db::getInstance()->execute($sql_update);
                //capuraLogs::nuevo_log("21) Cart sql_update : -{$sql_update}-");
            }
        } else {
            $sql       = "SELECT * FROM " . _DB_PREFIX_ . "cart_product WHERE id_cart = {$cart_id}";
            $productos = Db::getInstance()->executeS($sql);
            ...
            foreach ($productos as $producto) {
                ...
                $quantity             = $producto['quantity'];
                $sql                  = "SELECT * FROM " . _DB_PREFIX_ . "stock_global WHERE id_product = {$id_product} AND id_product_attribute ={$id_product_attribute} AND quantity >= {$quantity} AND sitio = '{$sitioCliente}'";
                //capuraLogs::nuevo_log("22) Cart sql : {$sql}");
                if (!Db::getInstance()->executeS($sql)) {
                    //capuraLogs::nuevo_log("23) Cart NOOOOO!");
                    $deOrigen = false;
                }
            ...
            if ($deOrigen) {
                $sql_update = "UPDATE " . _DB_PREFIX_ . "cart_product SET sitio = '{$sitioCliente}' WHERE id_cart = {$cart_id}";
                Db::getInstance()->execute($sql_update);
                //capuraLogs::nuevo_log("24) Cart sql_update : -{$sql_update}-");
            }
        }
        $sql = "DELETE FROM prstshp_sitios_productos WHERE id_cart = {$cart_id}";
        Db::getInstance()->execute($sql);
    }
    ...
    public function updateQty(
        ...
        if (!empty($cartProductQuantity['quantity'])) {
            $myQty               = $cartProductQuantity['quantity'];
            ...
            $mySitio = "";
            if ($myQty > 0) {
                $sql_select = "SELECT pcs.sitio FROM prstshp_cart pc ";
                $sql_select .= "INNER JOIN prstshp_customer pcs ON pcs.id_customer =  pc.id_customer AND pc.id_cart = {$this->id} ";
                $sql_select .= "INNER JOIN prstshp_stock_global psg ON psg.sitio = pcs.sitio AND psg.id_product = {$id_product} AND psg.id_product_attribute = {$id_product_attribute} AND psg.quantity >= {$myQty} ";
                $sitio = Db::getInstance()->executeS($sql_select);
                if ($sitio) {
                    foreach ($sitio as $sto) {
                        $mySitio = $sto['sitio'];
                    }
                } else {
                    $sql_select = "SELECT sitio FROM prstshp_stock_global psg WHERE sitio IN (SELECT sitio FROM prstshp_cart_product WHERE id_cart = {$this->id} GROUP BY sitio) AND quantity >= {$myQty} AND id_product = {$id_product} AND id_product_attribute = {$id_product_attribute}";
                    $sitio      = Db::getInstance()->executeS($sql_select);
                    if ($sitio) {
                        foreach ($sitio as $sto) {
                            $mySitio = $sto['sitio'];
                        ...
                        $sql_select = "SELECT sitio FROM prstshp_stock_global psg WHERE quantity >= {$myQty} AND id_product = {$id_product} AND id_product_attribute = {$id_product_attribute} ORDER BY quantity DESC LIMIT 1";
                        $sitio      = Db::getInstance()->executeS($sql_select);
                        if ($sitio) {
                            foreach ($sitio as $sto) {
                                $mySitio = $sto['sitio'];
            ...
            $sql = 'UPDATE `' . _DB_PREFIX_ . 'cart_product`
        SET `quantity` = `quantity` ' . $updateQuantity . ', sitio = \'' . $mySitio . '\'
        ...
        WHERE p.id_product = ' . $id_product;
            ...
            $sql_select = "SELECT sitio FROM prstshp_stock_global psg WHERE sitio IN (SELECT sitio FROM prstshp_cart_product WHERE id_cart = {$this->id} GROUP BY sitio) AND quantity >= {$quantity} AND id_product = {$id_product} AND id_product_attribute = {$id_product_attribute}";
            $sitio      = Db::getInstance()->executeS($sql_select);
            $mySitio    = "";
            if ($sitio) {
                foreach ($sitio as $sto) {
                ...
                $sql_select = "SELECT pcs.sitio FROM prstshp_cart pc ";
                $sql_select .= "INNER JOIN prstshp_customer pcs ON pcs.id_customer =  pc.id_customer AND pc.id_cart = {$this->id} ";
                $sql_select .= "INNER JOIN prstshp_stock_global psg ON psg.sitio = pcs.sitio AND psg.id_product = {$id_product} AND psg.id_product_attribute = {$id_product_attribute} AND psg.quantity >= {$quantity} ";
                $sitio = Db::getInstance()->executeS($sql_select);
                if ($sitio) {
                    foreach ($sitio as $sto) {
                        $mySitio = $sto['sitio'];
                    ...
                    $sql_select = "SELECT sitio FROM prstshp_stock_global psg WHERE quantity >= {$quantity} AND id_product = {$id_product} AND id_product_attribute = {$id_product_attribute} ORDER BY quantity DESC LIMIT 1";
                    $sitio      = Db::getInstance()->executeS($sql_select);
                    if ($sitio) {
                        foreach ($sitio as $sto) {
                            $mySitio = $sto['sitio'];
