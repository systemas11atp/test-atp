 ...
 *}
 ...
 <div class="form-group row {if !empty($field.errors)}has-error{/if}">
  ...
  <div class="col-md-6{if ($field.type === 'radio-buttons')} form-control-valign{/if}">
    ...
    {if $field.name eq 'id_state'} onchange="accionesDireccion(this,'state')"{/if}
    {if $field.name eq 'city'} onchange="accionesDireccion(this,'city')"{/if}
    {if $field.name eq 'colony'} onchange="accionesDireccion(this,'colony')"{/if}
    >
...
<span class="custom-checkbox">
  <label>
    ...
    <span><i class="material-icons mi checkbox-checked rtl-no-flip">&#xE5CA;</i></span>
...
{/block}
{else}
{if $field.name eq 'postcode' and {$field.value} ne ''}
<script>
  if ({$id_address}) {
     $( document ).ready(function() {
      accionesDireccion({$id_address},'edicion_direccion')
...
{if $field.required}required{/if}
{if $field.name eq 'postcode'} onkeyup="accionesDireccion(this,'zipcode')"{/if}
{if $field.name eq 'RFC'} onchange="checkRFC(this)"{/if}
>
{if $field.name eq 'RFC'}<div class="help-block" id="validateRFC" style="display: none;"><ul><li class="alert alert-danger">Formato no válido.</li></ul></div>{/if}
{if isset($field.availableValues.comment)}
<span class="form-control-comment">
  {$field.availableValues.comment}
</span>
