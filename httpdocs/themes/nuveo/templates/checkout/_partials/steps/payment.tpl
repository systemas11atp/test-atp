      ...
      {foreach from=$module_options item="option"}
        <div>
          <div id="{$option.id}-container" class="payment-option clearfix">
            ...
            <span class="custom-radio float-xs-left">
              <input
                ...
                {if $selected_payment_option == $option.id || $is_free} checked {/if}
                 onclick=" llamaAccion('{$option.id}', '{$option.call_to_action_text}','{$customer.id}','{$customer.bank_reference}')"
              >
              <span></span>
            </span>
            ...
            <label for="{$option.id}">
              <span>{if $option.call_to_action_text == 'Referencia'}Transferencia bancaria referenciada{else}{$option.call_to_action_text}{/if}</span>
              {if $option.logo}
                <img src="{$option.logo}">
              {/if}
            </label>
        ...
        {if $option.additionalInformation}
            ...
            <div id="{$option.id}-additional-information" class="js-additional-information definition-list additional-information{if $option.id != $selected_payment_option} ps-hidden {/if}" >
                ...
                {if $option.call_to_action_text=="Tarjeta"}  

                  <iframe id="iframeWebPay" src="https://wppsandbox.mit.com.mx/i/VX46ZI8M?"  scrolling="no" height="700px" style="width: 100%; overflow: hidden; border:0px;"></iframe>
